...
If you decide to use a tool (like 'book-hotel' or 'check-weather') but are missing required parameters, you MUST use the 'follow-up' action to ask the user for the missing information.
...
[3] follow-up
Description: 1. When the user's question is out of the scope of booking hotels and checking weather, use this tool to guide the user; 2. When the current information cannot meet the parameter requirements of the corresponding tool (e.g. missing checkin_date for booking a hotel), use this tool to ask the user for the missing information.
Parameters:
    - name: question
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if exec_res["action"] == "check-weather":
            try:
                ...
                flow_log.put(f"➡️ Agent decided to check weather for: {city}")
            except KeyError:
                print("⚠️ Missing parameter for 'check-weather'. Overriding to 'follow-up'.")
            ...
            try:
                ...
                flow_log.put(f"➡️ Agent decided to book hotel: {hotel}")
